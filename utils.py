...
def eval_input_seq2seq(
    ...
    psylex:torch.utils.data.Dataset=None,
    topN:int=2,
    isPrint:bool=True,
    ):
    ...
    inp_ids = psylex.source_tkn2ids(inp_wrd) + [psylex71.source_vocab.index('<EOW>')]
    inp_tensor = convert_ids2tensor(inp_ids)

    encoder.eval(); decoder.eval()
    enc_hid = encoder.initHidden()
    enc_outputs = torch.zeros(psylex71.target_maxlen, encoder.n_hid, device=device)
    ...
    dec_inp = torch.tensor([[psylex.target_vocab.index('<SOW>')]], device=device)
    dec_hid = enc_hid
    dec_wrds, dec_ids, dec_vals = [], [], []
    dec_attns = torch.zeros(psylex.target_maxlen, psylex.target_maxlen)
    for di in range(psylex.target_maxlen):
        ...
        if topi.squeeze().numpy()[0] == psylex.target_vocab.index('<EOW>'):
            break
        dec_inp = topi.detach().squeeze()[0]

    dec_wrds = psylex.target_ids2tkn(dec_ids)
...
#eval_input_seq2seq()
#eval_input_seq2seq(inp_wrd='北海道')
