    ...
    * <UNK>: 未定義トークン
    ...
    def __init__(self,
        ...
        for i, orth in enumerate(onechars):
            ...
            self.train_data[i] = {'orig': orth, 'orth': orth, 'yomi': _yomi,  'phon': phon_juli,
                            'phon_ids': phon_ids, 'orth_ids': orth_ids }
        ...
        self.set_source_and_target_from_params(source=source, target=target, is_print=is_Print)
    ...
    def make_ntt_freq_data(self, ps71_fname:str=None):
        ...
        ntt_freq = {x[0]-1: {
            '単語': x[1],
            #'単語': jaconv.normalize(x[1]),
            '品詞': x[2],
            '頻度': x[3],
            'よみ': jaconv.kata2hira(jaconv.normalize(x[4]))} for x in tmp2}
